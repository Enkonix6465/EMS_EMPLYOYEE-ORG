...
export default function AdvancedLeaveApplication() {
  ...
  const handleCancelLeave = async (leaveId: string) => {
    ...
    try {
      const leaveRef = doc(db, "leaveManage", leaveId);
  ...
  const isCancellable = (leave: LeaveRecord): boolean => {
    if (leave.status !== "pending" && leave.status !== "accepted") {
      return false;
    }

    const shiftStartTimeIST = 10; // 10:00 AM IST
    const istOffsetHours = 5.5;
    const shiftStartTimeUTC = shiftStartTimeIST - istOffsetHours; // 4.5 UTC

    const cancellationDeadlineHoursBefore = 4;

    const deadlineOffsetMilliseconds =
      (shiftStartTimeUTC - cancellationDeadlineHoursBefore) * 60 * 60 * 1000;

    // leave.date is 'YYYY-MM-DD', new Date() creates it at UTC midnight
    const leaveDateUTCMidnight = new Date(leave.date).getTime();
    const deadlineUTC = leaveDateUTCMidnight + deadlineOffsetMilliseconds;
  ...
  const tileClassName = ({ date, view }: { date: Date; view: string }) => {
    ...
    if (view === "month" && leave) {
      ...
      if (leave.status === "cancelled")
        return "bg-gray-400 dark:bg-gray-600 text-black dark:text-white line-through";
    }
    return null;
  };
        ...
        <table className="w-full text-sm table-auto border dark:border-gray-700">
          <thead className="bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200">
            <tr>
              ...
              <th className="border px-3 py-2 hidden md:table-cell">Applied On</th>
              <th className="border px-3 py-2 hidden md:table-cell">Time</th>
                  ...
                  <td className="border px-3 py-2">
                    {(leave.status === "pending" ||
                      ...
                      <button
                        ...
                        title={
                          !isCancellable(leave)
                            ? "Cannot cancel within 4 hours of shift start"
