// Updated Project Management with Search + Member Names from /employees
...
import { PlusCircle, Edit2, Trash2, Brain, AlertTriangle, TrendingUp, Target, Zap } from "lucide-react";
...
function Projects() {
  ...
  // AI Features State
  const [projectInsights, setProjectInsights] = useState<{[projectId: string]: AIInsight[]}>({});
  ...
  // AI Data Fetching
  const fetchAIProjectData = async (projectId: string) => {
    try {
      const insights = await analyzeProjectHealth(projectId);
  ...
  // AI Components
  const ProjectHealthWidget = ({ project }: { project: Project }) => {
    ...
    return (
      <div className="bg-gradient-to-r from-green-50 to-blue-50 dark:from-green-900/20 dark:to-blue-900/20 rounded-lg p-3 mb-3">
        <div className="flex items-center gap-2 mb-2">
          ...
          <h4 className="font-medium text-sm text-blue-900 dark:text-blue-100">AI Project Health</h4>
        </div>
        <div className="space-y-1">
          {insights.slice(0, 2).map((insight, index) => (
            <div key={index} className={`flex items-start gap-2 p-2 rounded text-xs ${
              ...
              {insight.type === 'alert' && <AlertTriangle className="h-3 w-3 text-red-600 mt-0.5 flex-shrink-0" />}
              {insight.type === 'optimization' && <Zap className="h-3 w-3 text-green-600 mt-0.5 flex-shrink-0" />}
              {insight.type === 'trend' && <TrendingUp className="h-3 w-3 text-blue-600 mt-0.5 flex-shrink-0" />}
  ...
  // Fetch AI data for projects when they load
  useEffect(() => {
    if (projects.length > 0) {
      projects.forEach(project => {
        fetchAIProjectData(project.id);
      });
    }
  }, [projects]);
